...
%edge map: sobel
[~,threshold] = edge(GUI.IM,'sobel');
fudgeFactor = 0.5;
E_1 = edge(imgaussfilt(GUI.IM, 1),'sobel', threshold * fudgeFactor);
se = strel('diamond',1);
E_2 = imdilate(E_1,[se se]);
E = imfilter(E_2,fspecial('gaussian'));

%Gaussian filter
intensityatseed = GUI.IM(GUI.YSeed,GUI.XSeed);
G = normpdf(GUI.IM, intensityatseed, GUI.Slider2);

%complete the solution by defining the Speed function
GUI.SPEED = (GUI.Slider1 * (0.01 * imcomplement(E))) + ((1 - GUI.Slider1) * G); 
