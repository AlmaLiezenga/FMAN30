...
title('Unsampled shape')

Xmcoord_temp= Xmcoord';
Xmcoord_temp(end+1) = Xmcoord_temp(1); 
Ymcoord_temp= Ymcoord'; 
Ymcoord_temp(end+1) = Ymcoord_temp(1); 
pt = interparc(15,Xmcoord_temp,Ymcoord_temp,'linear');
figure
imagesc(dmsa_images(:,:,pat_nbr))
colormap(gray)
axis xy
axis equal
hold on
pt_c = complex(pt(:,1),pt(:,2));
drawshape_comp(pt_c,[1 length(pt_c) 1],'.-r')
title('Sampled shape')
hold off
